...
Hupper.Menu.prototype = {
    ...
    add: function () {
        if (Hupper.HUP.El.GetId(this.id) || this.block) {
            return;
        }
        this.create();
    ...
    addMenuItem: function (menuItem, parent, first) {
      // {name: 'block name', href: 'false, http://...', click: function () {}}
        ...
        var li = Hupper.HUP.El.Li(),
            a = Hupper.HUP.El.CreateLink(menuItem.name, menuItem.href || 'javascript:void(0)');
        ...
        if (first && parent.firstChild) {
            Hupper.HUP.El.Insert(li, parent.firstChild);
        } else {
            Hupper.HUP.El.Add(li, parent);
        }
        if (this.hidden) {
            this.show();
        }
