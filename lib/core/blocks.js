...
function updateBlock (details, prefName, value, blockPrefs) {
	const block = func.first(blockPrefs, b => b.id === details.id);
...
function getBlockTitles (user) {
	return {
		...
		'block-user-1': `/user/${user.id}/track`,
	};
}

function getBlockTitle (block, user) {
	return getBlockTitles(user)[block.id];
}
