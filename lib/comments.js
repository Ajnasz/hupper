...
function getFirstParagraph(comment) {
	'use strict';
	return getParagraphs(comment)[0];
}
...
function parseComments(worker, comments) {
	...
	if (pref.getPref('hideboringcomments')) {
		boringComments = comments.filter(partial(isBorinComment, new RegExp(pref.getPref('boringcommentcontents'))));
