...
(function () {
	let tabs = new Set();
	let blockTabs = new Set();

	const toggleTwitterBlock = contentBlocker.getBlocker(blockTabs);

	prefs.getPref('block-twitter').then(toggleTwitterBlock);
	prefs.on('block-twitter', toggleTwitterBlock);
		...
		switch (event) {
			...
			case 'register':
				...
				Promise.all([
					...
					prefs.getPref('block-twitter')
				]).then(settings => {
					let [setunlimitedlinks, parseblocks, validateForms, logenabled, blockTwitter] = settings;
