...
function manageStyles (tabID) {
	...
	Promise.all([
		...
		prefs.getPref('style_min_comment_width'),
		prefs.getPref('style_hide_left_sidebar'),
		prefs.getPref('style_hide_right_sidebar'),
		prefs.getPref('style_accessibility')
	]).then((resp) => {
		const  [minFontSize, minWidth, minCommentWidth, hideLeftSidebar, hideRightSidebar, loadStyles] = resp;
		const styles = pageStyles.getPageStyle({ minFontSize, minWidth, minCommentWidth, hideLeftSidebar, hideRightSidebar });
		if (styles.length) {
