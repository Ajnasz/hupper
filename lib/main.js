...
function manageStyles (tabID) {
	...
	Promise.all([
		...
		prefs.getPref('style_max_indent_level'),
		prefs.getPref('style_hide_left_sidebar'),
		prefs.getPref('style_hide_right_sidebar'),
		prefs.getPref('style_accessibility')
	]).then((resp) => {
		const  [minFontSize, minWidth, maxIndentLevel, hideLeftSidebar, hideRightSidebar, loadStyles] = resp;
		const styles = pageStyles.getPageStyle({ minFontSize, minWidth, maxIndentLevel, hideLeftSidebar, hideRightSidebar });
		if (styles.length) {
