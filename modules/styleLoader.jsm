...
Storage.prototype = {
    ...
    styleIsActive_: function (aResultSet, onSuccess) {
        var styleActive = false, row, value;
        onSuccess = typeof onSuccess === 'function' ? onSuccess : function () {};
        while (row = aResultSet.getNextRow()) {
            value = row.getResultByName("active");
            styleActive = !!value;
        }
    ...
    styleAdded_: function (aResultSet, onSuccess) {
        var styleAdded = false, row, value;
        if (aResultSet) {
            while (row = aResultSet.getNextRow()) {
                value = row.getResultByName("count");
                styleAdded = value > 0;
            }
        }
...
var StyleLoader = function () {
    ...
    getURI = function (codeOrUri) {
        var output,
            path = codeOrUri.indexOf('chrome://') === 0;
        if (path) {
...
var EXPORTED_SYMBOLS = ['StyleLoader', 'styleLoader'];
