...
function updateBlock (details, prefName, value, blockPrefs) {
	let blockIndex = func.index(blockPrefs, b => b.id === details.id);

	blockPrefs[blockIndex] = Object.assign(blockPrefs[blockIndex], {[prefName]: value});

	return blockPrefs[blockIndex];
}

function getBlockTitle (block) {
	return blockTitles[block.id];
}
...
function mergeBlockPrefsWithBlocks (blocks, blocksPref) {
	...
	if (!blocksPref ||
		(!blocksPref.length &&
		(!blocksPref.items || !blocksPref.items.length))) {
		...
		const areSameBlocks = (block1, block2) => block1.id === block2.id;

		const leftBlocks = blocks.left
			.map(func.partial(createBlockPref, 'left'))
			.filter(block => !blocksPref.some(p => areSameBlocks(p, block)));

		const rightBlocks = blocks.right
			.map(func.partial(createBlockPref, 'right'))
			.filter(block => !blocksPref.some(p => areSameBlocks(p, block)));

		blocksPref = func.sortBy(blocksPref.concat(leftBlocks, rightBlocks), 'column');

		func
			.sortBy(blocksPref.filter(b => b.column === 'left'), 'index')
			.forEach((b, i) => b.index = i);
		func
			.sortBy(blocksPref.filter(b => b.column === 'right'), 'index')
			.forEach((b, i) => b.index = i);
	}
